...
Lifecycle metadata uses the `openinference.metadata` prefix, so
...
`nemo_relay.end.output`. LLM spans use OpenInference semantic input attributes
and final end-event metadata instead of the generic start projections. Mark
data, metadata, attributes, and category-profile fields use the corresponding
`nemo_relay.mark.*` prefixes. Nested objects and arrays remain JSON strings at
their top-level attribute, and `null` fields are omitted. The OpenInference
`metadata` JSON-string attribute is retained for backend compatibility. Use
