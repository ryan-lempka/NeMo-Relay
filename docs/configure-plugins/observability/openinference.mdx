...
NeMo Relay emits top-level `data` and `metadata` fields as typed dotted
attributes. For example, `metadata = { tenant = "acme" }` becomes
`openinference.metadata.tenant = "acme"`. Nested objects and arrays remain
JSON strings at their top-level attribute. Mark category-profile, handle, and
mark-attribute, input, and output fields flatten under
`nemo_relay.mark.category_profile`, `nemo_relay.handle_attributes`,
`nemo_relay.mark.attributes`, `nemo_relay.start.input`, and
`nemo_relay.end.output`. LLM inputs continue to use the OpenInference semantic
attributes. The OpenInference `metadata` JSON-string attribute is retained for
backend compatibility. Use `attribute_mappings` to copy a projected field to a
backend-specific alias.
