...
Top-level lifecycle payload fields are emitted as typed dotted attributes.
For example, `metadata = { tenant = "acme" }` becomes
`nemo_relay.start.metadata.tenant = "acme"`. Nested objects and arrays remain
JSON strings at their top-level attribute, and `null` fields are omitted. Start
and end `data`, `metadata`, `input`, and `output` fields use the corresponding
`nemo_relay.start.*` and `nemo_relay.end.*` prefixes. Handle attributes use
