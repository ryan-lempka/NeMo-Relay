...
Each stream sink receives the same raw ATOF JSON object that a file sink writes
as one JSONL line. Sinks are independent: a failed stream does not block file
output or other streams.
...
| `header_env` | `{}` | Header names mapped to environment variables that supply their values. |
  ...
  components: [
    observability.ComponentSpec({
      ...
      atof: observability.atofConfig({
        enabled: true,
        sinks: [
          { type: "file", output_directory: "logs", filename: "events.jsonl", mode: "overwrite" },
          {
            type: "stream",
            url: "http://localhost:8080/events",
            transport: "http_post",
...
use nemo_relay::observability::plugin_component::{
    AtofFileSinkSectionConfig, AtofSinkSectionConfig, AtofSectionConfig,
