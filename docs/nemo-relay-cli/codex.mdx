...
can record the parent and child activity but cannot recover the delegated task
text from the encrypted value.
...
To preserve readable delegated task content in traces, disable multi-agent v2
and use the legacy multi-agent path. Add the following setting to
`~/.codex/config.toml` for persistent Codex configuration:
...
This setting is a compatibility and observability workaround. It does not
decrypt previously captured values, and it does not change TLS protection for
traffic between Codex, NeMo Relay, and the upstream endpoint. Start a new Codex
process after changing the setting.
