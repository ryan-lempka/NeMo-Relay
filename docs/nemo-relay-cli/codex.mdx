...
When the selected Codex model and runtime allow the legacy multi-agent path,
disable multi-agent v2 to preserve readable delegated task content in new
traces. Add the following setting to `~/.codex/config.toml` for persistent Codex
configuration:
...
For a transparent run, pass the setting as a Codex root-level override:
...
Start a new Codex process after changing the setting. Confirm that Codex uses
the legacy tool surface because some model or product configurations can select
multi-agent v2 despite this feature setting. NeMo Relay cannot override that
selection or decrypt a v2 payload.

This setting is a compatibility and observability workaround. It affects only
newly captured traces, does not decrypt existing values, and does not change TLS
protection for traffic between Codex, NeMo Relay, and the upstream endpoint.
