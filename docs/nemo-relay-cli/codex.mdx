...
and Codex forwards that value to the child as
`agent_message.encrypted_content`. NeMo Relay observes the Responses API
traffic but does not hold the key that the Responses service uses to decrypt the
child message. As a result, ATOF, ATIF, OpenTelemetry, and OpenInference output
can contain observed parent and child activity but cannot recover the delegated
task text from the encrypted value. When Codex does not expose a lifecycle edge
for the built-in collaboration call, the parent and child can also appear as
separate trajectories instead of one linked delegation.
