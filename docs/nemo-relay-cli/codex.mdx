...
Codex multi-agent v2 marks the `spawn_agent` delegation message as encrypted in
the tool schema. The Responses service returns an opaque value in the tool call,
and Codex forwards that value to the child as an `encrypted_content` item in an
`agent_message`. NeMo Relay observes the Responses API traffic but does not hold
the key that the Responses service uses to decrypt the child message. As a
result, ATOF, ATIF, OpenTelemetry, and OpenInference output can contain observed
parent and child activity but cannot recover the delegated task text from the
encrypted value. When Codex does not expose a lifecycle edge for the built-in
collaboration call, the parent and child can also appear as separate
trajectories instead of one linked delegation.
...
upstream auditability limitation and
[NVIDIA/NeMo-Relay#425](https://github.com/NVIDIA/NeMo-Relay/issues/425) for the
NeMo Relay trace impact.

NeMo Relay disables multi-agent v2 for Relay-managed Codex sessions so that
Codex uses the legacy multi-agent path and new traces retain readable delegated
task content when the selected model and runtime support that path. You do not
need to disable the feature manually:

- `nemo-relay codex` and `nemo-relay run -- codex` inject the following
  root-level Codex override for that process:
  `features.multi_agent_v2.enabled=false`.
- `nemo-relay install codex` writes the equivalent setting to
  `~/.codex/config.toml`. Uninstalling the Codex integration restores the
  previous setting or removes the Relay-managed setting if none existed.

Confirm that Codex uses the legacy tool surface because a later user-provided
Codex override or an upstream model or product decision can still select
multi-agent v2. NeMo Relay cannot decrypt a v2 payload. Disabling the feature
affects only newly captured traces, does not decrypt existing values, and does
not change TLS protection for traffic between Codex, NeMo Relay, and the
upstream endpoint.
