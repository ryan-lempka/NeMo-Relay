...
outbox or delivery-deduplication journal. Opt-in end-to-end assertions verify
exactly-once lifecycle output for exercised healthy and cold-start paths, not
arbitrary process crashes between request acceptance and response delivery.
...
window. If the host rejects nested assignment, persistent bootstrap fails with
an actionable error instead of running without process-tree cleanup guarantees.
...
This release removes the internal `nemo-relay plugin-shim` command. Refresh
existing generated installations with `nemo-relay install <host> --force`.
Use the following public replacements for custom automation:
