...
These notes apply to the NVIDIA NeMo Relay 0.6 release.
...
- On Windows, a restrictive host Job Object can prevent a coding-agent plugin's
  ...
  the gateway remains scoped to the host job and might not remain available for
  the full 300-second idle reuse window. If the host rejects nested assignment,
  persistent bootstrap fails with an actionable error.
